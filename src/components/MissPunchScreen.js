...
function MissPunchCard({
  ...
  const missPunchDuration = (duration) => {
    const split = duration?.split(':');
    return `Duration ${split?.[0]}:${split?.[1]} hours`;
  };
    ...
    <VStack m={8} p={4} mx={6} my={2} bgColor="#fff" borderRadius="18px">
      <HStack py={2}>
        <Checkbox
          ...
          accessibilityLabel="select misspunch data"
          size="md"
          style={{
      ...
      <HStack style={{ opacity: isSelected ? 0.5 : 1 }}>
        <Text style={{ fontSize: 12, color: '#644A00' }}>
          ({missPunchDuration(item?.regularDuration)})
        </Text>
      </HStack>
      <HStack py={2} alignItems="center" style={{ opacity: isSelected ? 0.5 : 1 }}>
        ...
        <Text pl={1} fontWeight="semibold" style={{ color: '#212121', fontSize: 14 }}>
          {`${durationTimeFormat(item?.regfromtime)} - ${durationTimeFormat(item?.regtotime)}`}
        </Text>
      </HStack>
...
MissPunchCard.propTypes = {
  item: PropTypes.object.isRequired,
  setShowRejectPopup: PropTypes.func.isRequired,
  approveMissPunch: PropTypes.func.isRequired,
  isSelected: PropTypes.bool.isRequired,
  setSelectedData: PropTypes.func.isRequired,
  approverId: PropTypes.string.isRequired,
};

const MemoizedMissPunchCard = memo(MissPunchCard);
...
function MyApprovals() {
  ...
  const [approveMissPunch] = useApproveRegularizationMutation();
  ...
  const approveMissPunches = async (action, reason) => {
    const approvalData = {};
    approvalData.data = [];
    ...
    try {
      await approveMissPunch(approvalData).unwrap();
  ...
  useEffect(() => {
    setMissPunchData((l) => {
      ...
      if (filter === 'name') {
        ...
        filteredData?.sort(
          (a, b) =>
            new Date(`${b?.regdate?.split(' ')?.[0]}`) - new Date(`${a?.regdate?.split(' ')?.[0]}`),
        );
      } else if (filter === 'oldest') {
        filteredData?.sort(
          (a, b) =>
            new Date(`${a?.regdate?.split(' ')?.[0]}`) - new Date(`${b?.regdate?.split(' ')?.[0]}`),
        );
      }
      return filteredData;
    });
  ...
  useEffect(() => {
    ...
    setMissPunchData(() => {
      const data = missPunch?.filter((m) => m?.regstatus === 'WFMA');
        ...
        <View w="100%" h="100%" pb={3}>
          {isSelectAllLoading ? (
            ...
            <HStack mx={6} my={4} justifyContent="space-between" alignItems="center">
              <Checkbox
                ...
                accessibilityLabel="select all misspunch"
                isChecked={
                  missPunchData?.length === selectedData?.length && missPunchData?.length > 0
                }
                style={{
                  backgroundColor:
                    missPunchData?.length === selectedData?.length && missPunchData?.length > 0
              ...
              <HStack display={showBtn ? 'flex' : 'none'}>
                ...
                <RejectScreen
                  approveMissPunches={approveMissPunches}
                  showRejectPopup={showRejectPopup}
                  setShowRejectPopup={setShowRejectPopup}
                  setSelectedData={setSelectedData}
                />
                <Button
                  onPress={() => approveMissPunches(APPROVAL_ACTIONS?.APPROVE, '')}
                  bg="#000"
                  borderRadius={10}
          ...
          <FlashList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <MemoizedMissPunchCard
                  item={item}
                  isSelected={isSelected}
                  setSelectedData={setSelectedData}
                  approveMissPunch={approveMissPunch}
                  setShowRejectPopup={setShowRejectPopup}
...
export default function MissPunchScreen() {
  ...
  const { data: missPunch = [], isSuccess } = useGetRegularizationDataQuery(approverId, {
    skip: !approverId,
  });
          ...
          <HStack justifyContent="space-between">
            <HStack alignItems="center">
              ...
              <HStack
                ...
                px={2}
                justifyContent="center"
                display={missPunchData?.length === 0 ? 'none' : 'flex'}
                alignItems="center"
              ...
              >
                ...
                >
                  {missPunchData?.length || 0} Pending
                </Text>
              </HStack>
            </HStack>
            <HStack>
        ...
        <Modal.Content maxWidth="400px" style={{ borderRadius: 23, height: 285, width: 312 }}>
          <VStack px={4} py={6}>
            ...
            <HStack py={6} w="100%" justifyContent="space-between">
              ...
              <Button
                onPress={() => {
                  approveMissPunches(APPROVAL_ACTIONS?.REJECT, rejectReason);
                  setSelectedData([]);
                  setRejectReason('');
                }}
                bg="#F9B800"
...
RejectScreen.propTypes = {
  ...
  setShowRejectPopup: PropTypes.func.isRequired,
  approveMissPunches: PropTypes.func.isRequired,
  setSelectedData: PropTypes.func.isRequired,
};
